...
if (!grepl("CAPRI", getwd())) {
  ...
  if(mypc=="L01RI1203587"){ #checks machine name
    ...
    iam      <- 'ltEfisio' 
    locplots <- paste0(adrian,"eealocatorplots-master_original_test_on_20210315")  
...
if (!grepl("CAPRI", getwd())) {
  ...
  if(mypc=="L01RI1203587") {
    invloc  <-    paste0("C:/Adrian/google/projects/ecir")#!!!
  }
  ...
  if(iam=='ltEfisio') {
    invloc <- paste0(adrian,'ecir')
    csvfil <- paste0(locplots,"/../",invyear,"/eealocator/eealocator_",cursubm)   
    proc.dir <- paste0(invloc, '\\processing\\')
  }
...
plotsdir   <-paste0(invloc,"/plots") # modified by efisio
...
# set the last year of FAO report
year_last_report_fao <- '2018'
#
