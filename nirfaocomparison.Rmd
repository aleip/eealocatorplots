...
The Food and Agriculture Organization of the United Nations (FAO) has developed a database of greenhouse gas emissions, contained in FAOSTAT, which provides estimations  of the emissions of main gases in the agricultural sector (CH4 and N2O) and statistics on the activity data related to these emissions that generally cover ther period 1990-`r faOlast`. The data base can be consulted at the following link:
...
tabs("nirfaotab",paste0("GHG emissions from the agricultural sector by emission source category, in kt CO2-eq/year and % of total emissions, for the whole EU-28+ISL, averaged over the years ",comprange,", for which reported data from all countries are availabel in both the FAOSTAT and the UNFCCC data bases are available."))
...
Comparing both databases, we can see that UNFCCC reports higher total emissions than FAOSTAT (`r round(sum(curtable[1:(nrow(curtable)-1),3])/1000,1)` versus `r round(sum(curtable[1:(nrow(curtable)-1),5])/1000,1)`) Mt CO2-eq yr-1, but that difference is lower and with different sign if we remove from the UNFCCC total emissions the amount corresponding to 3.D.1.5, 3.G, 3.H and 3.I categories, which are not calculated in FAOSTAT (`r round(sum(curdiffs[,3])/1000,1)` versus `r round(sum(curdiffs[,4])/1000,1)`) Mt CO2-eq yr-1. Looking at the individual emission categories, we can also identify differences between the two databases, which can be due to different reasons: 
