...
euccountries<-curcountries[variable==eusubm & value==1]$code3
euacountries<-curcountries[variable==eusubm & value==1]$code3
...
agrigeneu<-dt2CO2eq(agrigen[party%in%c("EU28", "EUC")&meastype=="EM"&gas!="no gas",])
agrimixeu<-dt2CO2eq(agrimix[party%in%c("EU28", "EUC")&meastype=="EM"&gas!="no gas",])
agrideteu<-dt2CO2eq(agridet[party%in%c("EU28", "EUC")&meastype=="EM"&gas!="no gas",])
...
save(agrigeneu, agrimixeu, agrideteu, file=paste0(gsub("eealocator_", "agridata", csvfil), "_agriEUemissions.Rdata"))

v<-unique(agrigeneu$sector_number)
sel<-agrimixeu$category=="Farming"
agrimixeu$category[sel]<-paste(agrimixeu$classification[sel],agrimixeu$target[sel]," ")
x<-lapply(c(1:(length(v)-1)),function(x) makepie(piedata = agrimixeu,pieradius = 0.9,piename = "agrimixeu",piegrep = agrigeneu[party=="EUC"]$sector_number[x]))
