...
plotmeas<-plotmeas[plotmeas$meastype %in% c(meas2popweight,meas2mcf,meas2clima),]
...
for(imeas in c(1:nrow(plotmeas))){
    ...
    runfocus<-"value"
    if(runfocus=="value"){
        textiefval1<-"EU-KP value is obtained from a weighted average of country-values. "
