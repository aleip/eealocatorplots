...
if(!file.exists(faofile) | recalc_faofile == 1){
  ...
  t1<-extractuiddata.old(curagri,uid = seluids[1],c = allcountries,narm=FALSE,noeu=TRUE)
  t1[is.na(t1)]<-0
  for(i in c(2:length(seluids))){
    t2<-extractuiddata.old(curagri,uid = seluids[i],c = allcountries,narm=FALSE,noeu=TRUE)
    t2[is.na(t2)]<-0
    t1<-t1+t2
  }
  ...
  adt1<-extractuiddata.old(curagri,uid = seluids[1],c = allcountries,narm=FALSE,noeu=TRUE)
  adt1[is.na(adt1)]<-0
  for(i in c(2:length(seluids))){
    t2<-extractuiddata.old(curagri,uid = seluids[i],c = allcountries,narm=FALSE,noeu=TRUE)
    t2[is.na(t2)]<-0
  ...
  tmpallagri <- allagri
  allagri <- as.data.frame(allagri)
  faodata <- as.data.frame(faodata)
  ...
  faodata<-eu28sums(A = as.data.table(faodata),aeu = eusubm,years=years)
  ...
  faomeas<-(unique(as.data.frame(faodata)[,faounique]))
...
runfocus<-"value"
datasource <- c("nir","fao")
faodata<-filldf(as.data.table(faodata),names(allagri))
allagri <- tmpallagri #back to data.tbale
plotdatagenerated<-generateplotdata(rundata = rundata,datasource = datasource,subcountries = eusubm)
plotdata<-plotdatagenerated[[1]]
plotmeas<-data.frame(plotdatagenerated[[2]])
...
for(imeas in x1:x2){
  loopoverplots.1(imeas = imeas,runfocus = runfocus,eusubm = eusubm)
}
